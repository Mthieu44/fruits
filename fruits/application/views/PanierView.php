...
<body onmouseover="initSliders()">
    ...
    ?>
		...
		<div class="main">
			<h2 v-if="panier.length == 0" class="title">Votre panier est vide !</h2>
			<h2 v-if="panier.length == 1" class="title">Votre panier contient {{panier.length}} article</h2>
			<h2 v-if="panier.length > 1" class="title">Votre panier contient {{panier.length}} articles</h2>
			<div v-if="panier.length == 0" class="bestsellers">
				<div class="top-part">
					<hr class="line">
					<div class="top-text">
						<h2>Meilleures ventes</h2>
						<p class="p01">Remplissez votre panier avec une selection de nos {{meilleuresVentes.length}}
							meilleures ventes ! </p>
					</div>
				</div>
							...
							<div class="card card-product-top">
								...
								<div class="buttons">
									<div v-if="isIndisponible(fruit.id_fruit)">
										...
										<button class="add gray indispBouton"
											v-on:click="ajouterAuPanier(fruit.id_fruit)" disabled>Indisponible</button>
									</div>
									<div v-else>
										<div class="manage">
										...
										<button id="adding" class="add"
											v-on:click="ajouterAuPanier(fruit.id_fruit)">Ajouter
											au panier</button>
									</div>
			...
			<div class="totalPanier">
				...
				<div class="requireconn">
					<p> Vous devez être connecté pour valider votre commande </p>
					<a class="connexion" href="<?= site_url('Connexion') ?>"> Se connecter </a>
					<a class="connexion" href="<?= site_url('Connexion/register') ?>"> S'inscrire </a>
				</div>
				<?php else:?>
				<div v-if="panier.length == 0">
					<input type="button" value="Valider la commande"
						onclick="window.location.href='<?= site_url('Commande') ?>';" disabled />
				</div>
				<div v-else>
					<input type="button" value="Valider la commande"
						onclick="window.location.href='<?= site_url('Commande') ?>';" />
				</div>
				<?php endif ?>
			</div>
