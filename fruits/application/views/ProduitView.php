	...
	<div id="app-vue">
		...
		<div class="div">
			<a href="<?= site_url('Boutique') ?>">
				<p style="position:absolute; left:70px; top:110px;  ">
					< Retour </p> </a> <div class="flexible">
						<div class="top">
							<div v-if="isIndisponible(<?= $fruit->id_fruit ?>)">
								<img class="img indisp" src="<?= base_url('img/fruit/') . $fruit->image?>"
									alt="<?= $fruit->nom?>">
							</div>
							<div v-else>
								<img class="img" src="<?= base_url('img/fruit/') . $fruit->image?>"
									alt="<?= $fruit->nom?>">
							</div>
							<div class="rightsquare">
								<h1><?= $fruit->nom?></h1>
								<div class="barh"></div>
								<h2>Prix : <?= $GLOBALS['calculator']->calculatePrice($fruit->prix)?>
									<?= $GLOBALS['calculator']->getCurrency() ?></h2>
									...
									<div v-for="fruit in fruits" v-bind:key="fruit.id_fruit"
										...
										<div v-if="isIndisponible(<?= $fruit->id_fruit ?>)">
											<div class="plusoumoins grayD">
												<button v-on:click="totalQuantity(-1,fruit.id_fruit)"
													disbaled>-</button>
												<p>{{fruit.quantity}}</p>
												<button v-on:click="totalQuantity(1,fruit.id_fruit)" disabled>+</button>
											</div>
											<button id="adding" class="add gray "
												v-on:click="ajouterAuPanier(fruit.id_fruit)"
										...
										<div v-else>
											...
											<button id="adding" class="add"
												v-on:click="ajouterAuPanier(fruit.id_fruit)">Ajouter au panier</button>
										</div>
